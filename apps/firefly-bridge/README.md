...
Connect the Firefly Pixie device, then:
...
Flash `FIREFLY_DEVICE_PRIVATE_KEY` onto the device and set `FIREFLY_PUBLIC_KEY`
in the API's environment. Then:

```bash
FIREFLY_DEVICE_PATH=COM3 FIREFLY_PUBLIC_KEY=<hex> npm run dev:bridge  # http://localhost:4747
```
