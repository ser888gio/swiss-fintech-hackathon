...
def _load_ed25519():
    try:
        ...
        return Ed25519PrivateKey, Ed25519PublicKey, Encoding, NoEncryption, PublicFormat, PrivateFormat
    except ImportError:
        return None
...
class AuditEvent:
    ...
    prior_event_hash: str  # sha256 of the prior event's canonical JSON ("genesis" for first)
...
def configure(signing_key_hex: str | None = None) -> None:
    ...
    Call once from app startup. If None or empty, mock mode generates a
    deterministic key so tests always pass.
    """
    global _signing_key_bytes
    if signing_key_hex:
        _signing_key_bytes = bytes.fromhex(signing_key_hex)
    else:
        # Deterministic mock key: sha256("ars-mock-signing-key")
...
def verify(event: AuditEvent) -> bool:
    ...
    Ed25519PrivateKey, Ed25519PublicKey, Encoding, NoEncryption, PublicFormat, PrivateFormat = mods
    try:
        private_key = Ed25519PrivateKey.from_private_bytes(_signing_key_bytes)
        public_key = private_key.public_key()
        public_key.verify(bytes.fromhex(event.signature), bytes.fromhex(event.event_hash))
        return True
    except Exception:
        return False
...
def reset_mock_state() -> None:
    """Clear in-memory log for test isolation."""
    _events.clear()
...
def _sign(event_hash: str) -> str:
    mods = _load_ed25519()
    if mods is None or not _signing_key_bytes:
        return hashlib.sha256(f"mock-sig:{event_hash}".encode()).hexdigest()
    ...
    try:
        ...
        log.warning("Ed25519 signing failed, using mock sig: %s", exc)
        return hashlib.sha256(f"mock-sig:{event_hash}".encode()).hexdigest()
