...
> **Project adapter (read first).** This official Ripple skill treats Python
> (`xrpl-py`) and TypeScript (`xrpl.js`) as equally first-class. In **this**
> repository they are not: per `AGENTS.md`/`CLAUDE.md`, **every transaction this
> skill constructs is built and submitted with Python `xrpl-py`** inside
> `apps/api/app/tools/` (routing, execution, escrow). Do not introduce `xrpl.js`,
> `xrpl4j`, or a browser/bridge XRPL client. Use the TypeScript snippets only as
> behavioural reference and port them to `xrpl-py`. Settlement asset is RLUSD /
> issued token on Testnet (testnet RLUSD issuer `rQhWct2fv4Vc4KRjRgMrxa8xPN9Zx9iLKV`);
> never call a self-issued token RLUSD. Escrow (TokenEscrow XLS-85) is the lock
> primitive for large payments — the issuer must have enabled trust-line locking.
> Set `Memos` for the audit trail and keep `SourceTag = 20260530`. This skill
> builds the transaction object; the **xrpl-agent-wallet** skill (also `xrpl-py`
> here) signs and submits it behind the policy + Firefly boundary.
