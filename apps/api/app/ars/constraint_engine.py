...
class XRPLConstraintEngine(ConstraintEngine):
    ...
    async def evaluate(
        ...
        *,
        ...
        kya_status = await agent_credentials.verify_agent_kya(agent_address, required_scope)
        g1_passed = kya_status.verified and kya_status.scope_ok
        g1_reason = kya_status.reason if not g1_passed else None
        trail.append(GuardrailOutcome(
            name="G1_kya",
            passed=g1_passed,
        ...
        trail.append(GuardrailOutcome(
            ...
            reason=f"AML score {aml_score} ≥ threshold {threshold}" if not g3_passed else None,
        ))
        if not g3_passed:
            reasons.append(f"G3 AML: score {aml_score} exceeds flag threshold {threshold}")
            return ConstraintResult(
                allowed=False,
                action="review",
        ...
        if delegation_budget_remaining is not None:
            ...
            trail.append(GuardrailOutcome(
                ...
                reason=f"Amount {amount} exceeds delegation budget {delegation_budget_remaining}" if not g5_passed else None,
            ))
            if not g5_passed:
            ...
            trail.append(GuardrailOutcome(name="G5_delegation", passed=True, reason="not applicable"))
        ...
        if allowed_service_hosts and service_host:
            ...
            trail.append(GuardrailOutcome(
                ...
                reason=f"Host '{service_host}' not in allowlist" if not g6_passed else None,
            ))
            if not g6_passed:
            ...
            trail.append(GuardrailOutcome(name="G6_service_host", passed=True, reason="no host restriction"))
        ...
        trail.append(GuardrailOutcome(
            ...
            reason=f"Amount {amount} exceeds agent max {max_usd}" if not g7_passed else None,
        ))
        if not g7_passed:
