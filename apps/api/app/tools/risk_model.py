...
def evaluate_signals(
    *,
    ...
    purpose_is_vague = (
        ...
        or purpose_lower.replace(" ", "").replace("_", "")
        in {v.replace(" ", "") for v in _VAGUE_PURPOSES}
    )
    if purpose_is_vague:
        signals.append(
            RiskSignal(
    ...
    reference_is_missing = len(reference_lower.strip()) < 3 or reference_lower in (
        "n/a",
        "na",
        "none",
        "-",
